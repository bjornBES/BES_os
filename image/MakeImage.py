...
from partitons import create_partition_table
from sh import guestmount, Command, mmd, mcopy, fusermount
import parted
...
from build_scripts.config import imageType, imageFS, imageSize

stage1 = os.path.join(project_root, "build/stage1.bin")
stage2 = os.path.join(project_root, "build/stage2.bin")
kernel = os.path.join(project_root, "build/kernel.bin")
BASEDIR=""
...
def build_floppy(image, stage1, stage2, kernel, files):
    size_sectors = 2880
    ...
    for file in files:
        ...
        if os.path.isdir(file_src):
            ...
            mmd('-i', image, file_dst)
        else:
            print('    ... copying', file_rel)
            mcopy('-i', image, file_src, file_dst)


def mount_fs(image: str, mount_dir: str):
    print(f"image {image}")
    print(f"mount_dir {mount_dir}")
    guestmount(mount_dir, add=image, mount='/dev/sda1')
...
def build_disk(image, stage1, stage2, kernel, files):
    ...
    # install stage1
    print(f"> installing stage1...")
    install_stage1(image, stage1, offset=0, stage2_offset=1, stage2_size=stage2_sectors)
...
def build_image(target : str, source : list):
    stage1 = str(source[0])
    stage2 = str(source[1])
    kernel = str(source[2])
    files = source[3:]
    ...
    image = str(target)
    if imageType == 'floppy':
        build_floppy(image, stage1, stage2, kernel, files)
    elif imageType == 'disk':
        build_disk(image, stage1, stage2, kernel, files)
    else:
        raise ValueError('Unknown image type ' + imageType)
