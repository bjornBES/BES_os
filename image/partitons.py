...
def GetSectorNumber(file, image, stage1_offset):
    ...
    data_sector : int = initial_sector + first_data_sector
    data_offset : int = file_size % SECTOR_SIZE
    
    print(f"first_data_sector = {first_data_sector}")
    print(f"data_sector = {data_sector}")
...
def getRootDir(target: str, stage1_offset):
    ...
    with os.fdopen(os.open(target, os.O_RDWR | os.O_CREAT), 'rb') as ftarget:
        if imageFS == "fat32" or imageFS == "ext4":
            ...
            print(f"{BPB_RsvdSecCnt} + {BPB_FAT} * {BPB_NumFATs}")
