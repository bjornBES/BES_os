...
void rtc_read_datetime()
{
    ...
    current_datetime.second = readRTCRegister(0x00);
    current_datetime.minute = readRTCRegister(0x02);
    current_datetime.hour = readRTCRegister(0x04);
    current_datetime.day = readRTCRegister(0x07);
    current_datetime.month = readRTCRegister(0x08);
    current_datetime.year = readRTCRegister(0x09);
    ...
    if (!(registerB & 0x04))
    {
        current_datetime.second = from_bcd(current_datetime.second);
        current_datetime.minute = from_bcd(current_datetime.minute) * 10;
        current_datetime.hour = from_bcd(current_datetime.hour) | (current_datetime.hour & 0x80);
        current_datetime.day = from_bcd(current_datetime.day);
        current_datetime.month = from_bcd(current_datetime.month);
        current_datetime.year = from_bcd(current_datetime.year);
    }
}
...
int get_weekday_from_date(datetime *dt)
{
    ...
    int leap_year_code = is_leap_year((int)dt->year, (int)dt->month);
