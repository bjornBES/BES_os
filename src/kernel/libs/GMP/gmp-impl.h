...
/* must be given on any global data that will be accessed
   ...
   being used for the accesses.  On functions makes calls
...
#include <limits.h>
#include "hal/vfs.h" // For fd_t, vfs_node_t, etc.
#include "math.h"

/* For fat.h and other fat binary stuff.
   No need for or , since functions
...
#define DECL_gcd_11(name) \
    mp_limb_t name(mp_limb_t, mp_limb_t)
...
#define DECL_mod_1_1p(name) \
    mp_limb_t name(mp_srcptr, mp_size_t, mp_limb_t, const mp_limb_t[])
#define DECL_mod_1_1p_cps(name) \
    void name(mp_limb_t cps[], mp_limb_t b)
...
#define DECL_mullo_basecase(name) \
    void name(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t)
...
#define DECL_redc_1(name) \
    mp_limb_t name(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t)
#define DECL_redc_2(name) \
    mp_limb_t name(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr)
...
void *__gmp_tmp_alloc(unsigned long) ATTRIBUTE_MALLOC;
void __gmp_tmp_mark(struct tmp_marker *);
void __gmp_tmp_free(struct tmp_marker *);
#define TMP_SDECL TMP_DECL
...
void __gmp_tmp_debug_mark(const char *, int, struct tmp_debug_t **,
                          struct tmp_debug_t *,
                          const char *, const char *);
void *__gmp_tmp_debug_alloc(const char *, int, int,
                            struct tmp_debug_t **, const char *,
                            size_t) ATTRIBUTE_MALLOC;
void __gmp_tmp_debug_free(const char *, int, int,
                          struct tmp_debug_t **,
                          const char *, const char *);
...
extern void *(*gmp_allocate_func)(size_t);
extern void *(*gmp_reallocate_func)(void *, size_t, size_t);
extern void (*gmp_free_func)(void *, size_t);

void *gmp_default_allocate(size_t);
void *gmp_default_reallocate(void *, size_t, size_t);
...
#define __GMP_ALLOCATE_FUNC_TYPE(n, type) \
    ((type *)(*gmp_allocate_func)((n) * sizeof(type)))
...
#define __GMP_REALLOCATE_FUNC_TYPE(p, old_size, new_size, type) \
    ((type *)(*gmp_reallocate_func)(p, (old_size) * sizeof(type), (new_size) * sizeof(type)))
#define __GMP_REALLOCATE_FUNC_LIMBS(p, old_size, new_size) \
    __GMP_REALLOCATE_FUNC_TYPE(p, old_size, new_size, mp_limb_t)

#define __GMP_FREE_FUNC_TYPE(p, n, type) (*gmp_free_func)(p, (n) * sizeof(type))
...
#define __GMP_REALLOCATE_FUNC_MAYBE(ptr, oldsize, newsize)         \
    ...
    {                                                              \
        if ((oldsize) != (newsize))                                \
            (ptr) = (*gmp_reallocate_func)(ptr, oldsize, newsize); \
    } while (0)
...
#define __GMP_REALLOCATE_FUNC_MAYBE_TYPE(ptr, oldsize, newsize, type)                                        \
    ...
    {                                                                                                        \
        if ((oldsize) != (newsize))                                                                          \
            (ptr) = (type *)(*gmp_reallocate_func)(ptr, (oldsize) * sizeof(type), (newsize) * sizeof(type)); \
    } while (0)
...
void __gmpz_aorsmul_1(REGPARM_3_1(mpz_ptr, mpz_srcptr, mp_limb_t, mp_size_t)) REGPARM_ATTR(1);
#define mpz_aorsmul_1(w, u, v, sub) __gmpz_aorsmul_1(REGPARM_3_1(w, u, v, sub))

void mpz_n_pow_ui(mpz_ptr, mp_srcptr, mp_size_t, unsigned long);

// #define mp\w_[\w]+ __gmp\w_[\w_]+\([\w,]+\)\n
mp_limb_t mpn_addmul_1c(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_addmul_2(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_3(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_4(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_5(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_6(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_7(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

mp_limb_t mpn_addmul_8(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);
...
mp_limb_t mpn_divexact_by255(mp_ptr dst, mp_srcptr src, mp_size_t size);
...
mp_limb_t mpn_addlsh1_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif
#ifndef mpn_addlsh1_n_ip1
mp_limb_t mpn_addlsh1_n_ip1(mp_ptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_addlsh1_nc_ip1
mp_limb_t mpn_addlsh1_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif

#ifndef mpn_addlsh2_n
mp_limb_t mpn_addlsh2_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_addlsh2_nc
mp_limb_t mpn_addlsh2_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif
#ifndef mpn_addlsh2_n_ip1
mp_limb_t mpn_addlsh2_n_ip1(mp_ptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_addlsh2_nc_ip1
mp_limb_t mpn_addlsh2_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif

#ifndef mpn_addlsh_n
mp_limb_t mpn_addlsh_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int);
#endif
#ifndef mpn_addlsh_nc
mp_limb_t mpn_addlsh_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int, mp_limb_t);
#endif
#ifndef mpn_addlsh_n_ip1
mp_limb_t mpn_addlsh_n_ip1(mp_ptr, mp_srcptr, mp_size_t, unsigned int);
#endif
#ifndef mpn_addlsh_nc_ip1
mp_limb_t mpn_addlsh_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, unsigned int, mp_limb_t);
#endif

#ifndef mpn_sublsh1_n
mp_limb_t mpn_sublsh1_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_sublsh1_nc
mp_limb_t mpn_sublsh1_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif
#ifndef mpn_sublsh1_n_ip1
mp_limb_t mpn_sublsh1_n_ip1(mp_ptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_sublsh1_nc_ip1
mp_limb_t mpn_sublsh1_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif

#ifndef mpn_sublsh2_n
mp_limb_t mpn_sublsh2_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_sublsh2_nc
mp_limb_t mpn_sublsh2_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif
#ifndef mpn_sublsh2_n_ip1
mp_limb_t mpn_sublsh2_n_ip1(mp_ptr, mp_srcptr, mp_size_t);
#endif
#ifndef mpn_sublsh2_nc_ip1
mp_limb_t mpn_sublsh2_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);
#endif

#ifndef mpn_sublsh_n
mp_limb_t mpn_sublsh_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int);
#endif
#ifndef mpn_sublsh_nc
mp_limb_t mpn_sublsh_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int, mp_limb_t);
#endif
#ifndef mpn_sublsh_n_ip1
mp_limb_t mpn_sublsh_n_ip1(mp_ptr, mp_srcptr, mp_size_t, unsigned int);
#endif
#ifndef mpn_sublsh_nc_ip1
mp_limb_t mpn_sublsh_nc_ip1(mp_ptr, mp_srcptr, mp_size_t, unsigned int, mp_limb_t);
#endif

mp_limb_signed_t mpn_rsblsh1_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
mp_limb_signed_t mpn_rsblsh1_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_signed_t mpn_rsblsh2_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
mp_limb_signed_t mpn_rsblsh2_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_signed_t mpn_rsblsh_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int);
mp_limb_signed_t mpn_rsblsh_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, unsigned int, mp_limb_t);

mp_limb_t mpn_rsh1add_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
mp_limb_t mpn_rsh1add_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_rsh1sub_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);
mp_limb_t mpn_rsh1sub_nc(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
...
mp_limb_t mpn_add_err3_n(mp_ptr, mp_srcptr, mp_srcptr, mp_ptr, mp_srcptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);
...
mp_limb_t mpn_sub_err3_n(mp_ptr, mp_srcptr, mp_srcptr, mp_ptr, mp_srcptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_add_n_sub_n(mp_ptr, mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);

mp_limb_t mpn_add_n_sub_nc(mp_ptr, mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_addaddmul_1msb0(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_divrem_1c(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);

void mpn_dump(mp_srcptr, mp_size_t);

mp_size_t mpn_fib2_ui(mp_ptr, mp_ptr, unsigned long);

int mpn_fib2m(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

int mpn_strongfibo(mp_srcptr, mp_size_t, mp_ptr);

/* Remap names of internal mpn functions.  */
int mpn_jacobi_base(mp_limb_t, mp_limb_t, int) ATTRIBUTE_CONST;

int mpn_jacobi_2(mp_srcptr, mp_srcptr, unsigned);

int mpn_jacobi_n(mp_ptr, mp_ptr, mp_size_t, unsigned);

mp_limb_t mpn_mod_1c(mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_mul_1c(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);
...
mp_limb_t mpn_mul_6(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

void mpn_mul_basecase(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

void mpn_mullo_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);

void mpn_mullo_basecase(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);

void mpn_sqr_basecase(mp_ptr, mp_srcptr, mp_size_t);

void mpn_sqrlo(mp_ptr, mp_srcptr, mp_size_t);

void mpn_sqrlo_basecase(mp_ptr, mp_srcptr, mp_size_t);

void mpn_mulmid_basecase(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

void mpn_mulmid_n(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);

void mpn_mulmid(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

mp_limb_t mpn_submul_1c(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_redc_1(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_redc_2(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

void mpn_redc_n(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr);

void mpn_mod_1_1p_cps(mp_limb_t[4], mp_limb_t);
mp_limb_t mpn_mod_1_1p(mp_srcptr, mp_size_t, mp_limb_t, const mp_limb_t[4]);

void mpn_mod_1s_2p_cps(mp_limb_t[5], mp_limb_t);
mp_limb_t mpn_mod_1s_2p(mp_srcptr, mp_size_t, mp_limb_t, const mp_limb_t[5]);

void mpn_mod_1s_3p_cps(mp_limb_t[6], mp_limb_t);
mp_limb_t mpn_mod_1s_3p(mp_srcptr, mp_size_t, mp_limb_t, const mp_limb_t[6]);

void mpn_mod_1s_4p_cps(mp_limb_t[7], mp_limb_t);
mp_limb_t mpn_mod_1s_4p(mp_srcptr, mp_size_t, mp_limb_t, const mp_limb_t[7]);

void mpn_bc_mulmod_bnm1(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_ptr);
void mpn_mulmod_bnm1(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mulmod_bnm1_next_size(mp_size_t) ATTRIBUTE_CONST;
static inline mp_size_t mpn_mulmod_bnm1_itch(mp_size_t rn, mp_size_t an, mp_size_t bn)
{
    mp_size_t n, itch;
    n = rn >> 1;
    itch = rn + 4 +
...
void mpn_sqrmod_bnm1(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_sqrmod_bnm1_next_size(mp_size_t) ATTRIBUTE_CONST;
...
void mpn_sqr_diag_addlsh1(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t);

void mpn_toom_interpolate_5pts(mp_ptr, mp_ptr, mp_ptr, mp_size_t, mp_size_t, int, mp_limb_t);
...
void mpn_toom_interpolate_6pts(mp_ptr, mp_size_t, enum toom6_flags, mp_ptr, mp_ptr, mp_ptr, mp_size_t);
...
void mpn_toom_interpolate_7pts(mp_ptr, mp_size_t, enum toom7_flags, mp_ptr, mp_ptr, mp_ptr, mp_ptr, mp_size_t, mp_ptr);

void mpn_toom_interpolate_8pts(mp_ptr, mp_size_t, mp_ptr, mp_ptr, mp_size_t, mp_ptr);

void mpn_toom_interpolate_12pts(mp_ptr, mp_ptr, mp_ptr, mp_ptr, mp_size_t, mp_size_t, int, mp_ptr);

void mpn_toom_interpolate_16pts(mp_ptr, mp_ptr, mp_ptr, mp_ptr, mp_ptr, mp_size_t, mp_size_t, int, mp_ptr);

void mpn_toom_couple_handling(mp_ptr, mp_size_t, mp_ptr, int, mp_size_t, int, int);

int mpn_toom_eval_dgr3_pm1(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_size_t, mp_ptr);

int mpn_toom_eval_dgr3_pm2(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_size_t, mp_ptr);

int mpn_toom_eval_pm1(mp_ptr, mp_ptr, unsigned, mp_srcptr, mp_size_t, mp_size_t, mp_ptr);

int mpn_toom_eval_pm2(mp_ptr, mp_ptr, unsigned, mp_srcptr, mp_size_t, mp_size_t, mp_ptr);

int mpn_toom_eval_pm2exp(mp_ptr, mp_ptr, unsigned, mp_srcptr, mp_size_t, mp_size_t, unsigned, mp_ptr);

int mpn_toom_eval_pm2rexp(mp_ptr, mp_ptr, unsigned, mp_srcptr, mp_size_t, mp_size_t, unsigned, mp_ptr);

void mpn_toom22_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom32_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom42_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom52_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom62_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom2_sqr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom33_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom43_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom53_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom54_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom63_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom3_sqr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom44_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom4_sqr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom6h_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom6_sqr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom8h_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom8_sqr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);

void mpn_toom42_mulmid(mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_ptr);

int mpn_fft_best_k(mp_size_t, int) ATTRIBUTE_CONST;

mp_limb_t mpn_mul_fft(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, int);

void mpn_mul_fft_full(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

void mpn_nussbaumer_mul(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);

mp_size_t mpn_fft_next_size(mp_size_t, int) ATTRIBUTE_CONST;

mp_limb_t mpn_div_qr_1n_pi1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_div_qr_2n_pi1(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t, mp_limb_t);

mp_limb_t mpn_div_qr_2u_pi1(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t, int, mp_limb_t);

mp_limb_t mpn_sbpi1_div_qr(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_sbpi1_div_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_sbpi1_divappr_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_dcpi1_div_qr(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, gmp_pi1_t *);
mp_limb_t mpn_dcpi1_div_qr_n(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, gmp_pi1_t *, mp_ptr);

mp_limb_t mpn_dcpi1_div_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, gmp_pi1_t *);

mp_limb_t mpn_dcpi1_divappr_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, gmp_pi1_t *);

mp_limb_t mpn_mu_div_qr(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mu_div_qr_itch(mp_size_t, mp_size_t, int) ATTRIBUTE_CONST;

mp_limb_t mpn_preinv_mu_div_qr(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_preinv_mu_div_qr_itch(mp_size_t, mp_size_t, mp_size_t) ATTRIBUTE_CONST;

mp_limb_t mpn_mu_divappr_q(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mu_divappr_q_itch(mp_size_t, mp_size_t, int) ATTRIBUTE_CONST;

mp_limb_t mpn_mu_div_q(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mu_div_q_itch(mp_size_t, mp_size_t, int) ATTRIBUTE_CONST;
...
mp_limb_t mpn_ni_invertappr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);
mp_limb_t mpn_invertappr(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);
#define mpn_invertappr_itch(n) (2 * (n))

void mpn_binvert(mp_ptr, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_binvert_itch(mp_size_t) ATTRIBUTE_CONST;

mp_limb_t mpn_bdiv_q_1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_pi1_bdiv_q_1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t, int);

mp_limb_t mpn_sbpi1_bdiv_qr(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

void mpn_sbpi1_bdiv_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_sbpi1_bdiv_r(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_dcpi1_bdiv_qr(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);
mp_size_t mpn_dcpi1_bdiv_qr_n_itch(mp_size_t) ATTRIBUTE_CONST;

mp_limb_t mpn_dcpi1_bdiv_qr_n(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_ptr);
void mpn_dcpi1_bdiv_q(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t);

mp_limb_t mpn_mu_bdiv_qr(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mu_bdiv_qr_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

void mpn_mu_bdiv_q(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_mu_bdiv_q_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

mp_limb_t mpn_bdiv_qr(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_bdiv_qr_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

void mpn_bdiv_q(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_bdiv_q_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

void mpn_divexact(mp_ptr, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t);
mp_size_t mpn_divexact_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

#ifndef mpn_bdiv_dbm1c /* if not done with cpuvec in a fat binary */
mp_limb_t mpn_bdiv_dbm1c(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);
#endif

// mp_limb_t mpn_bdiv_dbm1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t)
#define mpn_bdiv_dbm1(dst, src, size, divisor) \
    mpn_bdiv_dbm1c((mp_ptr)dst, (mp_srcptr)src, (mp_size_t)size, (mp_limb_t)divisor, __GMP_CAST(mp_limb_t, 0))
...
mp_limb_t mpn_sec_pi1_div_qr(mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t, mp_ptr);
void mpn_sec_pi1_div_r(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t, mp_ptr);
...
void mpz_divexact_gcd(mpz_ptr, mpz_srcptr, mpz_ptr);

mp_size_t mpz_prodlimbs(mpz_ptr, mp_ptr, mp_size_t);

void mpz_oddfac_1(mpz_ptr, mp_limb_t, unsigned);

int mpz_stronglucas(mpz_srcptr, mpz_ptr, mpz_ptr);

int mpz_lucas_mod(mpz_ptr, mpz_ptr, long, mp_bitcnt_t, mpz_srcptr, mpz_ptr, mpz_ptr);
...
mp_size_t mpn_rootrem(mp_ptr, mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);

void mpn_broot(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);

void mpn_broot_invm1(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t);

void mpn_brootinv(mp_ptr, mp_srcptr, mp_size_t, mp_limb_t, mp_ptr);

void mpn_bsqrt(mp_ptr, mp_srcptr, mp_bitcnt_t, mp_ptr);

int mpn_bsqrtinv(mp_ptr, mp_srcptr, mp_bitcnt_t, mp_ptr);
...
/* Initialize X of type mpz_struct* with space for NLIMBS limbs.  X should be a
...
void gmp_init_primesieve(gmp_primesieve_t *);

#define gmp_nextprime __gmp_nextprime
unsigned long int gmp_nextprime(gmp_primesieve_t *);

#define gmp_primesieve __gmp_primesieve
mp_limb_t gmp_primesieve(mp_ptr, mp_limb_t);
...
#else
#define ASSERT(expr) assert(expr) /* NOP, but still evaluates expr */
#endif
...
mp_limb_t mpn_trialdiv(mp_srcptr, mp_size_t, mp_size_t, int *);

mp_bitcnt_t mpn_remove(mp_ptr, mp_size_t *, mp_srcptr, mp_size_t, mp_srcptr, mp_size_t, mp_bitcnt_t);
...
extern struct bases mp_bases[257];
...
// size_t mpn_sizeinbase (size_t result, mp_srcptr ptr, mp_size_t size, int base)
size_t mpn_sizeinbase(mp_srcptr ptr, mp_size_t size, int base);

// void MPN_SIZEINBASE_2EXP (size_t result, mp_srcptr ptr, mp_size_t size, unsigned long base2exp)
...
/* MPN_SET_UI sets an mpn (ptr, cnt) to given ui.  MPZ_FAKE_UI creates fake
   mpz_struct* from ui.  The zp argument must have room for LIMBS_PER_ULONG limbs
...
mp_limb_t mpn_preinv_divrem_1(mp_ptr, mp_size_t, mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t, int);
#endif
...
mp_limb_t mpn_modexact_1c_odd(mp_srcptr, mp_size_t, mp_limb_t, mp_limb_t);
#endif
...
extern unsigned char binvert_limb_table[128];
...
double mpn_get_d(mp_srcptr, mp_size_t, mp_size_t, long);
...
mp_double_limb_t mpn_gcd_22(mp_limb_t, mp_limb_t, mp_limb_t, mp_limb_t);
...
extern const unsigned char jacobi_table[208];
...
void mpn_matrix22_mul(mp_ptr, mp_ptr, mp_ptr, mp_ptr, mp_size_t, mp_srcptr, mp_srcptr, mp_srcptr, mp_srcptr, mp_size_t, mp_ptr);
mp_size_t mpn_matrix22_mul_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;
...
int mpn_hgcd2(mp_limb_t, mp_limb_t, mp_limb_t, mp_limb_t, struct hgcd_matrix1 *);

mp_size_t mpn_hgcd_mul_matrix1_vector(const struct hgcd_matrix1 *, mp_ptr, mp_srcptr, mp_ptr, mp_size_t);

mp_size_t mpn_matrix22_mul1_inverse_vector(const struct hgcd_matrix1 *, mp_ptr, mp_srcptr, mp_ptr, mp_size_t);

int mpn_hgcd2_jacobi(mp_limb_t, mp_limb_t, mp_limb_t, mp_limb_t, struct hgcd_matrix1 *, unsigned *);
...
void mpn_hgcd_matrix_init(struct hgcd_matrix *, mp_size_t, mp_ptr);

void mpn_hgcd_matrix_update_q(struct hgcd_matrix *, mp_srcptr, mp_size_t, unsigned, mp_ptr);

void mpn_hgcd_matrix_mul_1(struct hgcd_matrix *, const struct hgcd_matrix1 *, mp_ptr);

void mpn_hgcd_matrix_mul(struct hgcd_matrix *, const struct hgcd_matrix *, mp_ptr);

mp_size_t mpn_hgcd_matrix_adjust(const struct hgcd_matrix *, mp_size_t, mp_ptr, mp_ptr, mp_size_t, mp_ptr);

mp_size_t mpn_hgcd_step(mp_size_t, mp_ptr, mp_ptr, mp_size_t, struct hgcd_matrix *, mp_ptr);

mp_size_t mpn_hgcd_reduce(struct hgcd_matrix *, mp_ptr, mp_ptr, mp_size_t, mp_size_t, mp_ptr);

mp_size_t mpn_hgcd_reduce_itch(mp_size_t, mp_size_t) ATTRIBUTE_CONST;

mp_size_t mpn_hgcd_itch(mp_size_t) ATTRIBUTE_CONST;

mp_size_t mpn_hgcd(mp_ptr, mp_ptr, mp_size_t, struct hgcd_matrix *, mp_ptr);

mp_size_t mpn_hgcd_appr_itch(mp_size_t) ATTRIBUTE_CONST;

int mpn_hgcd_appr(mp_ptr, mp_ptr, mp_size_t, struct hgcd_matrix *, mp_ptr);

mp_size_t mpn_hgcd_jacobi(mp_ptr, mp_ptr, mp_size_t, struct hgcd_matrix *, unsigned *, mp_ptr);
...
mp_size_t mpn_gcd_subdiv_step(mp_ptr, mp_ptr, mp_size_t, mp_size_t, gcd_subdiv_step_hook *, void *, mp_ptr);
...
mp_size_t mpn_gcdext_lehmer_n(mp_ptr, mp_ptr, mp_size_t *, mp_ptr, mp_ptr, mp_size_t, mp_ptr);
...
size_t mpn_compute_powtab(powers_t *, mp_ptr, mp_size_t, int);
mp_size_t mpn_dc_set_str(mp_ptr, const unsigned char *, size_t, const powers_t *, mp_ptr);
...
int __gmp_asprintf_memory(struct gmp_asprintf_t *, const char *, size_t);
int __gmp_asprintf_reps(struct gmp_asprintf_t *, int, int);
int __gmp_asprintf_final(struct gmp_asprintf_t *);
...
int __gmp_doprnt(const struct doprnt_funs_t *, void *, const char *, va_list);
int __gmp_doprnt_integer(const struct doprnt_funs_t *, void *, const struct doprnt_params_t *, const char *);

#define __gmp_doprnt_mpf __gmp_doprnt_mpf2
int __gmp_doprnt_mpf(const struct doprnt_funs_t *, void *, const struct doprnt_params_t *, const char *, mpf_srcptr);

int __gmp_replacement_vsnprintf(char *, size_t, const char *, va_list);
...
#if _GMP_H_HAVE_VA_LIST
int __gmp_doscan(const struct gmp_doscan_funs_t *, void *, const char *, va_list);
#endif
...
/* Enhancement: The "mod" and "gcd_1" functions below could have
  , but currently (gcc 3.3) that's not supported on
...
extern int __gmpn_cpuvec_initialized;
