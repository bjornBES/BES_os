...
#ifndef have_mpn_divexact_by255
#define have_mpn_divexact_by255
mp_limb_t mpn_divexact_by255(mp_ptr dst, mp_srcptr src, mp_size_t size) 
{
  mp_limb_t ret = mpn_bdiv_dbm1(dst, src, size, __GMP_CAST(mp_limb_t, GMP_NUMB_MASK / 255));
  return (mp_limb_t)((mp_limb_t)255 & (mp_limb_t)1 * ret);
}
#else
...
void mpn_toom_interpolate_16pts(mp_ptr pp, mp_ptr r1, mp_ptr r3, mp_ptr r5, mp_ptr r7,
