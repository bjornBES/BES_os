...
void mpz_n_pow_ui(mpz_ptr r, mp_srcptr bp, mp_size_t bsize, unsigned long int e)
{
  mp_ptr rp;
 ...
 probably faster than mul_2 (FIXME: is this true?)  */
    ...
    if (rtwos_bits != 0 && !(rl_high == 0 && rl == 1) && (rl_high >> (GMP_NUMB_BITS - rtwos_bits)) == 0)
    {
      mp_limb_t new_rl_high = (rl_high << rtwos_bits) | (rl >> (GMP_NUMB_BITS - rtwos_bits));
 ...
 - rl mustn't overflow	*/
    ...
    if (rtwos_bits != 0 && rl != 1 && (rl >> (GMP_NUMB_BITS - rtwos_bits)) == 0)
    {
      rl <<= rtwos_bits;
      rtwos_bits = 0;
