...
randget_lc(gmp_randstate_t rstate, mp_ptr rp, unsigned long int nbits)
{
  ...
  while (rbitpos + chunk_nbits <= nbits)
  {
    ...
    if (rbitpos % GMP_NUMB_BITS != 0)
    {
      ...
      r2p[0] |= savelimb;
      /* bogus */
      if ((chunk_nbits % GMP_NUMB_BITS + rbitpos % GMP_NUMB_BITS) > GMP_NUMB_BITS)
        r2p[tn] = rcy;
    }
    else
    {
  ...
  if (rbitpos != nbits)
  {
    ...
    if (nbits % GMP_NUMB_BITS != 0)
      rp[nbits / GMP_NUMB_BITS] &= ~(~CNST_LIMB(0) << nbits % GMP_NUMB_BITS);
  }
...
randclear_lc(gmp_randstate_t rstate)
{
  ...
  mpz_clear(p->_mp_a);
  (*gmp_free_func)(p, sizeof(gmp_rand_lc_struct));
}
...
randiset_lc(gmp_randstate_ptr dst, gmp_randstate_srcptr src)
{
  ...
  dstp = (gmp_rand_lc_struct *)(*gmp_allocate_func)(sizeof(gmp_rand_lc_struct));
...
void gmp_randinit_lc_2exp(gmp_randstate_t rstate,
                          mpz_srcptr a,
                          unsigned long int c,
                          mp_bitcnt_t m2exp)
{
