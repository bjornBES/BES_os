...
int cob_check_numval_f(const cob_field *srcfield)
{
...
{
    return (toupper(p[0]) == 'C' && toupper(p[1]) == 'R') || (toupper(p[0]) == 'D' && toupper(p[1]) == 'B');
}
...
numval(cob_field *srcfield, cob_field *currency, const enum numval_type type)
{
    ...
    {
        if (space_left(p, p_end) >= 2 && at_cr_or_db(p))
        {
        ...
        if (currency_data)
        {
            if (space_left(p, p_end) >= currency->size && !memcmp(p, currency_data, currency->size))
            {
                if (currency_seen)
                {
                    exception = 1;
        ...
        default:
            ...
            else if (*p == num_sep && type == NUMVAL_C)
            {
...
{
    char *scanf_str = format.with_colons ? "%2d:%2d:%2d" : "%2d%2d%2d";
    int hours;
    int minutes;
    int seconds;
    int total_seconds;
...
{
    char *scanf_str = format.with_hyphens ? "%2d-%2d" : "%2d%2d";
    int month;
    int day;
                ...
                const char *final_part)
{
    ...
    char *scanf_str = format.with_hyphens ? "W%2d-%1d" : "W%2d%1d";
    int week;
    int day_of_week;
...
void cob_decimal_pow(cob_decimal *pd1, cob_decimal *pd2)
{
    ...
    if (!pd2->scale)
    {
        ...
        if (mpz_sgn(pd2->value) == -1 && mpz_fits_slong_p(pd2->value))
        {
            /* Negative power */
...
int cob_check_numval(const cob_field *srcfield, const cob_field *currency,
                     const int chkcurr, const int anycase)
{
    ...
    for (; n < (int)max_pos; ++n, ++p)
    {
        ...
        case '-':
            ...
            p++;
            n++; /* trailing +/-, only space afterwards allowed */
            break_needed = 1;
            break;
        case 'c':
            if (!anycase)
...
{
    return !strcmp(format, "YYYYMMDD") || !strcmp(format, "YYYY-MM-DD") || !strcmp(format, "YYYYDDD") || !strcmp(format, "YYYY-DDD") || !strcmp(format, "YYYYWwwD") || !strcmp(format, "YYYY-Www-D");
}
...
int cob_valid_time_format(const char *format, const char decimal_point)
{
    ...
    if (format[format_offset] == decimal_point)
    {
        ...
        if (decimal_places == 0 || decimal_places > COB_TIMEDEC_MAX)
        {
            return 0;
        }
    }
    ...
    if (strlen(format) > (size_t)format_offset && !rest_is_z(format + format_offset) && !rest_is_offset_format(format + format_offset, with_colons))
    {
        return 0;
    }
...
int cob_valid_datetime_format(const char *format, const char decimal_point)
{
    ...
    if (!cob_valid_date_format(date_format_str) || !cob_valid_time_format(time_format_str, decimal_point))
    {
        return 0;
    }
...
cob_field *cob_intr_bit_of(cob_field *srcfield)
{
    cob_field field;
...
cob_field *cob_intr_bit_to_char(cob_field *srcfield)
{
    cob_field field;
    const size_t size = srcfield->size / 8;
...
cob_field *cob_intr_hex_of(cob_field *srcfield)
{
...
cob_field *cob_intr_hex_to_char(cob_field *srcfield)
{
    cob_field field;
    const size_t size = srcfield->size / 2;
...
cob_field *cob_intr_concatenate(const int offset, const int length,
                                const int params, ...)
{
    cob_field **f;
    unsigned char *p;
...
cob_field *cob_intr_substitute(const int offset, const int length,
                               const int params, ...)
{
    cob_field *ret;
    va_list args;
...
cob_field *cob_intr_substitute_case(const int offset, const int length,
                                    const int params, ...)
{
    cob_field *ret;
    va_list args;
...
cob_field *cob_intr_trim(const int offset, const int length,
...
cob_field *cob_intr_random(const int params, ...)
{
    ...
    else if (rand_needs_seeding)
    {
        ...
        seed = get_seconds_past_midnight() * (((cob_s64_t)COB_MODULE_PTR) & 0xFFFFF);
#else
        seed = get_seconds_past_midnight() * (((cob_s32_t)COB_MODULE_PTR) & 0xFFFF);
#endif
...
cob_field *cob_intr_date_to_yyyymmdd(const int params, ...)
{
    ...
    if (year < 0 || year > 999999 || !valid_year(current_year) || (maxyear < 1700 || maxyear > 9999))
    {
        cob_set_exception(COB_EC_ARGUMENT_FUNCTION);
        cob_alloc_set_field_uint(0);
        return curr_field;
...
cob_field *cob_intr_locale_date(const int offset, const int length,
...
cob_field *cob_intr_locale_time(const int offset, const int length,
                                cob_field *srcfield, cob_field *locale_field)
{
#if defined(HAVE_LANGINFO_CODESET)
    unsigned char *p;
...
cob_field *cob_intr_lcl_time_from_secs(const int offset, const int length,
                                       cob_field *srcfield, cob_field *locale_field)
{
#if defined(HAVE_LANGINFO_CODESET)
    int indate;
...
cob_field *cob_intr_lowest_algebraic(cob_field *srcfield)
{
    ...
    case COB_TYPE_NUMERIC_COMP5:
        ...
        if (COB_FIELD_REAL_BINARY(srcfield) || !COB_FIELD_BINARY_TRUNC(srcfield))
        {
...
cob_field *cob_intr_formatted_date(const int offset, const int length,
...
cob_field *cob_intr_formatted_time(const int offset, const int length,
                                   const int params, ...)
{
    va_list args;
...
cob_field *cob_intr_formatted_datetime(const int offset, const int length,
                                       const int params, ...)
{
    va_list args;
    cob_field *fmt_field;
                                            ...
                                            cob_field *datetime_field)
{
    ...
    if (date_present && time_present && formatted_datetime[strlen(formatted_date)] != 'T')
    {
...
cob_field *cob_intr_formatted_current_date(const int offset, const int length,
                                           cob_field *format_field)
{
    cob_field field;
...
**/
cob_field *cob_intr_content_length(cob_field *srcfield)
{
...
**/
cob_field *cob_intr_content_of(const int offset, const int length, const int params, ...)
{
    size_t size = 0;
    unsigned char *pointed;
...
cob_field *cob_intr_display_of(const int offset, const int length,
                               const int params, ...)
{
    COB_UNUSED(offset);
    COB_UNUSED(length);
