...
void cob_binary_set_uint64(cob_field *f, cob_u64_t n)
{
#ifndef WORDS_BIGENDIAN
    if (COB_FIELD_BINARY_SWAP(f))
    {
...
void cob_binary_set_int64(cob_field *f, cob_s64_t n)
{
#ifndef WORDS_BIGENDIAN
    if (COB_FIELD_BINARY_SWAP(f))
    {
...
void cob_decimal_init2(cob_decimal *d, const cob_uli_t initial_num_bits)
{
...
void cob_decimal_clear(cob_decimal *d)
{
    if (d)
    {
        mpz_clear(d->value);
...
void cob_decimal_set_ullint(cob_decimal *d, const cob_u64_t n)
{
#ifdef COB_LI_IS_LL
...
const cob_uli_t cob_pow_10_uli_val[MAX_LI_DIGITS_PLUS_1] = {
    1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000UL
#ifdef COB_LI_IS_LL
    ,
    10000000000, 100000000000, 1000000000000, 10000000000000, 100000000000000, 1000000000000000, 10000000000000000, 100000000000000000, 1000000000000000000, 10000000000000000000UL
#endif
};
...
cob_decimal_adjust(cob_decimal *d, mpz_struct *max_value, int min_exp, int max_exp)
{
    ...
    if (mpz_cmpabs(d->value, max_value) > 0 || d->scale < min_exp || d->scale > max_exp)
    {
...
void cob_decimal_set_mpf(cob_decimal *d, const mpf_t src)
{
    if (!mpf_sgn(src))
    {
...
void cob_decimal_get_mpf(mpf_t dst, const cob_decimal *d)
{
...
int cob_packed_get_sign(const cob_field *f)
{
...
cob_decimal_get_binary(cob_decimal *d, cob_field *f, const int opt)
{
    ...
    overflow = 0;
    if (!field_sign && mpz_sgn(d->value) == -1)
    {
        mpz_abs(d->value, d->value);
    }
...
void cob_decimal_set_field(cob_decimal *dec, cob_field *field)
{
...
int cob_decimal_get_field(cob_decimal *d, cob_field *f, const int opt)
{
    if (unlikely(d->scale == COB_DECIMAL_NAN))
    {
        if (!cobglobptr->cob_exception_code || !cob_last_exception_is(COB_EC_SIZE_ZERO_DIVIDE))
        {
            cob_set_exception(COB_EC_SIZE_OVERFLOW);
        }
        return cobglobptr->cob_exception_code;
    ...
    case COB_TYPE_NUMERIC_FLOAT:
    {
        ...
        if ((opt & COB_STORE_KEEP_ON_OVERFLOW) && (isinf(fval) || isnan(fval)))
        {
            cob_set_exception(COB_EC_SIZE_OVERFLOW);
            return cobglobptr->cob_exception_code;
        }
        if ((opt & COB_STORE_KEEP_ON_OVERFLOW) && cob_not_finite)
        {
            cob_set_exception(COB_EC_SIZE_OVERFLOW);
            return cobglobptr->cob_exception_code;
        }
    ...
    case COB_TYPE_NUMERIC_DOUBLE:
    {
        ...
        if ((opt & COB_STORE_KEEP_ON_OVERFLOW) && (isinf(val) || isnan(val)))
        {
            cob_set_exception(COB_EC_SIZE_OVERFLOW);
            return cobglobptr->cob_exception_code;
        }
...
void cob_decimal_set(cob_decimal *dst, cob_decimal *src)
{
 ...
 */
void cob_decimal_align(cob_decimal *d1, const int scale)
{
    if (d1->scale > scale)
    {
...
int cob_addsub_optimized(cob_field *f1, cob_field *f2,
                         const int opt, const enum cob_statement stmt)
{
    ...
    if (f1_type == COB_TYPE_NUMERIC_PACKED)
    {
        ...
        else
        {
            ...
            if ((f2_type == COB_TYPE_NUMERIC_DISPLAY && f2_digits <= COB_MAX_DIGITS) || f2_type == COB_TYPE_NUMERIC_BINARY || f2_type == COB_TYPE_NUMERIC_COMP5)
            {
                cob_field field;
                cob_field_attr attr;
...
void cob_div_quotient(cob_field *dividend, cob_field *divisor,
...
void cob_move_bcd(cob_field *f1, cob_field *f2)
{
    /************************************************************/
    /*                                                          */
...
int handle_bcd_rounding(int *byte, const int round_half_nibble,
...
int count_leading_zeros(unsigned char *ptr_char, int len)
{
    int cntr = 0;
                            ...
                            int final_positive, unsigned char *buff, unsigned char *pos)
{
    ...
    if (cmp_size >= 0)
    {
        ...
        const int has_pad_digit =
            (COB_FIELD_DIGITS(f) % 2 == 1 && COB_FIELD_NO_SIGN_NIBBLE(f)) || (COB_FIELD_DIGITS(f) % 2 == 0 && !COB_FIELD_NO_SIGN_NIBBLE(f));
        ...
        if ((cmp_size > 0)    /* more data than possible to store - definitely an overflow */
            || (cmp_size == 0 /* check if we only expect a single digit but have two set */
                && has_pad_digit && *(pos + lead_zeros) > 0x0F))
        {
            /* Overflow */
            if ((opt & COB_STORE_NO_SIZE_ERROR) == 0)
            {
            ...
            int opt, const enum cob_statement stmt)
{
    ...
    const signed short used_original_scale = src1_scale > src2_scale ? src1_scale : src2_scale;
    ...
    const signed short used_scale = fld1_scale > fld2_scale ? fld1_scale : fld2_scale;
    ...
    if ((used_original_scale <= dst_scale) || (used_scale < dest_scale) || (opt & COB_STORE_TRUNCATION) || !(opt & ~COB_STORE_MASK))
    {
        check_rounding = 0;
        check_all_zeros = 0;
        all_zeros = 0;
    ...
    else
    {
        ...
        if (opt & (COB_STORE_AWAY_FROM_ZERO | COB_STORE_PROHIBITED | COB_STORE_TOWARD_GREATER | COB_STORE_TOWARD_LESSER | COB_STORE_NEAR_TOWARD_ZERO))
        {
            check_all_zeros = 1;
            all_zeros = 1;
        }
    ...
    if (fld1_sign == fld2_sign || (fld1_sign != 0x0D && fld2_sign != 0x0D))
    {
...
int cob_add_int(cob_field *f, const int n, const int opt)
{
    ...
    if (type >= COB_TYPE_NUMERIC_FLOAT && type <= COB_TYPE_NUMERIC_FP_BIN128)
    {
    ...
    const int both_are_negative)
{
    ...
    if ((scale1 < scale2) || (scale1 == scale2 && no_sign_nibble_f1))
    {
...
int cob_bcd_cmp(cob_field *f1, cob_field *f2)
{
...

    {
        ...
        if (f1->size == f2->size /* note: we explicit ignore different digits here */
            && no_sign_nibble_f1 == no_sign_nibble_f2 && scale1 == scale2)
        {
...
int cob_numeric_cmp(cob_field *f1, cob_field *f2)
{
    ...
    if (f1_type == COB_TYPE_NUMERIC_FLOAT || f1_type == COB_TYPE_NUMERIC_DOUBLE || f1_type == COB_TYPE_NUMERIC_L_DOUBLE || f2_type == COB_TYPE_NUMERIC_FLOAT || f2_type == COB_TYPE_NUMERIC_DOUBLE || f2_type == COB_TYPE_NUMERIC_L_DOUBLE)
    {
    ...
    if (f1_type == COB_TYPE_NUMERIC_PACKED && f2_type == COB_TYPE_NUMERIC_PACKED)
    {
    ...
    if (COB_FIELD_SCALE(f1) == COB_FIELD_SCALE(f2) && COB_FIELD_DIGITS(f1) < 19 && COB_FIELD_DIGITS(f2) < 19)
    {
...
int cob_cmp_numdisp(const unsigned char *data, const size_t size,
...
/* BIT-WISE functions */
void cob_logical_not(cob_decimal *d0, cob_decimal *d1)
{
...
void cob_logical_or(cob_decimal *d0, cob_decimal *d1)
{
...
void cob_logical_and(cob_decimal *d0, cob_decimal *d1)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
    const cob_u64_t ur = u0 & u1;
...
void cob_logical_xor(cob_decimal *d0, cob_decimal *d1)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
    const cob_u64_t ur = u0 ^ u1;
...
void cob_logical_left(cob_decimal *d0, cob_decimal *d1)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
    const cob_u64_t ur = u0 << u1;
...
void cob_logical_right(cob_decimal *d0, cob_decimal *d1)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
    const cob_u64_t ur = u0 >> u1;
...
void cob_logical_left_c(cob_decimal *d0, cob_decimal *d1, int bytes)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
...
void cob_logical_right_c(cob_decimal *d0, cob_decimal *d1, int bytes)
{
    const cob_u64_t u0 = mpz_get_ui(d0->value);
    const cob_u64_t u1 = mpz_get_ui(d1->value);
    const cob_u64_t ur = (u0 >> u1) | (u0 << ((cob_u64_t)bytes * 8 - u1));
