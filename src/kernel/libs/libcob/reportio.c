...
free_control_fields(cob_report *r)
{
    ...
    for (rc = r->controls; rc; rc = rc->next)
    {
        ...
        {
            if (rr->ref_line->flags & COB_REPORT_CONTROL_HEADING || rr->ref_line->flags & COB_REPORT_CONTROL_HEADING_FINAL)
                rc->has_heading = TRUE;
            if (rr->ref_line->flags & COB_REPORT_CONTROL_FOOTING || rr->ref_line->flags & COB_REPORT_CONTROL_FOOTING_FINAL)
                rc->has_footing = TRUE;
        }
    }
...
{
    while (l && l->fields == NULL && l->child != NULL)
...
dumpFlags(int flags, int ln, char *name)
{
    ...
    if (name == NULL)
        name = (char *)"";
    if (flags & COB_REPORT_HEADING)
        DEBUG_LOG("rw", ("REPORT HEADING "));
    if (flags & COB_REPORT_FOOTING)
        DEBUG_LOG("rw", ("REPORT FOOTING "));
    if (flags & COB_REPORT_PAGE_HEADING)
        DEBUG_LOG("rw", ("PAGE HEADING "));
    if (flags & COB_REPORT_PAGE_FOOTING)
        DEBUG_LOG("rw", ("PAGE FOOTING "));
    if (flags & COB_REPORT_CONTROL_HEADING)
        DEBUG_LOG("rw", ("CONTROL HEADING %s ", name));
    if (flags & COB_REPORT_CONTROL_HEADING_FINAL)
        DEBUG_LOG("rw", ("CONTROL HEADING FINAL "));
    ...
    if (flags & COB_REPORT_CONTROL_FOOTING_FINAL)
        DEBUG_LOG("rw", ("CONTROL FOOTING FINAL "));
    if (flags & COB_REPORT_DETAIL)
        DEBUG_LOG("rw", ("DETAIL "));
    if (flags & COB_REPORT_LINE_PLUS)
    {
        if (ln > 0)
            DEBUG_LOG("rw", ("LINE PLUS %d ", ln));
    }
    else if (flags & COB_REPORT_LINE)
        DEBUG_LOG("rw", ("LINE %d ", ln));
    if (flags & COB_REPORT_LINE_NEXT_PAGE)
        DEBUG_LOG("rw", ("LINE NEXT PAGE "));
    if (flags & COB_REPORT_NEXT_PAGE)
        DEBUG_LOG("rw", ("NEXT PAGE "));
    if (flags & COB_REPORT_GROUP_INDICATE)
        DEBUG_LOG("rw", ("GROUP INDICATE "));
    if (flags & COB_REPORT_COLUMN_PLUS)
        DEBUG_LOG("rw", ("COLUMN PLUS "));
    if (flags & COB_REPORT_RESET_FINAL)
        DEBUG_LOG("rw", ("RESET FINAL "));
    if (flags & COB_REPORT_COLUMN_LEFT)
        DEBUG_LOG("rw", ("LEFT "));
    if (flags & COB_REPORT_COLUMN_RIGHT)
        DEBUG_LOG("rw", ("RIGHT "));
    if (flags & COB_REPORT_COLUMN_CENTER)
        DEBUG_LOG("rw", ("CENTER "));
    if (flags & COB_REPORT_GROUP_ITEM)
        DEBUG_LOG("rw", ("GROUP "));
    if (flags & COB_REPORT_PRESENT)
    {
        ...
        if (flags & COB_REPORT_PAGE)
            DEBUG_LOG("rw", ("PAGE "));
        if (flags & COB_REPORT_ALL)
            DEBUG_LOG("rw", ("ALL "));
    }
    else if (flags & COB_REPORT_HAD_WHEN)
        DEBUG_LOG("rw", ("WHEN "));
}
#endif
...
reportDumpOneLine(const cob_report *r, cob_report_line *fl, int indent, int dumpdata)
{
    ...
    if (fl->step_count)
        DEBUG_LOG("rw", ("Step %3d ", fl->step_count));
    if (fl->suppress)
        DEBUG_LOG("rw", ("Suppress Line "));
    if (fl->next_group_line)
    {
        DEBUG_LOG("rw", ("NEXT ", fl->next_group_line));
        if (fl->report_flags & COB_REPORT_NEXT_GROUP_LINE)
            DEBUG_LOG("rw", ("GROUP LINE "));
        if (fl->report_flags & COB_REPORT_NEXT_GROUP_PLUS)
            DEBUG_LOG("rw", ("GROUP PLUS "));
        if (fl->report_flags & COB_REPORT_NEXT_GROUP_PAGE)
            DEBUG_LOG("rw", ("GROUP PAGE "));
        DEBUG_LOG("rw", ("%d ", fl->next_group_line));
    }
    else
    {
        if (fl->report_flags & COB_REPORT_NEXT_GROUP_PAGE)
            DEBUG_LOG("rw", ("NEXT GROUP PAGE "));
    }
    if (fl->control)
    {
    ...
    DEBUG_LOG("rw", ("\n"));
    if (!(fl->flags & COB_REPORT_DETAIL))
        dumpdata = 1;
    for (rf = fl->fields; rf; rf = rf->next)
    {
        DEBUG_LOG("rw", ("%s   %02d Field ", idnt, rf->level));
        if (rf->line)
            DEBUG_LOG("rw", ("Line %2d ", rf->line));
        if (rf->column)
            DEBUG_LOG("rw", ("Col %3d ", rf->column));
        if (rf->step_count)
            DEBUG_LOG("rw", ("Step %3d ", rf->step_count));
        if (rf->next_group_line)
            DEBUG_LOG("rw", ("NextGrp %d ", rf->next_group_line));
        if (dumpdata)
        {
            ...
            if (rf->source && cob_cmp(rf->f, rf->source) != 0)
            {
            ...
            if ((rf->flags & COB_REPORT_PRESENT) && !rf->present_now && r->initiate_done)
            {
            ...
            else if ((rf->flags & COB_REPORT_GROUP_ITEM) && rf->suppress)
            {
            ...
            if (rf->control && (!(rf->flags & COB_REPORT_PRESENT) || rf->present_now || !r->initiate_done))
            {
                strcpy(wrk, "");
                for (rc = r->controls; rc; rc = rc->next)
                {
        ...
        if (!(rf->flags & COB_REPORT_GROUP_ITEM) && rf->suppress)
...
reportDump(const cob_report *r, const char *msg)
{
    ...
    if (r->report_file)
    {
        ...
        if (r->report_file->assign && r->report_file->assign->data)
        {
...
line_control_one(cob_report *r, cob_report_line *l, cob_field *f)
{
    ...
    for (rf = l->fields; rf; rf = rf->next)
    {
        ...
        if (!(rf->flags & COB_REPORT_NEGATE) && !rf->present_now)
        {
            if (f == NULL)
            { /* New Page */
                DEBUG_LOG("rw", ("PRESENT NOW: %s NEW PAGE\n", fld));
                if (rf->flags & COB_REPORT_PAGE)
                { /* PRESENT After New Page */
                    rf->present_now = 1;
                }
            }
            else if (rf->control == f)
            { /* Control field changed */
                DEBUG_LOG("rw", ("PRESENT NOW: %s control change\n", fld));
                rf->present_now = 1;
            }
        }
        else if ((rf->flags & COB_REPORT_NEGATE) && rf->present_now)
        {
            if (f == NULL)
            { /* New Page */
...
write_rec(cob_report *r, int opt)
{
    ...
    if (r->code_is_present && r->code_len > 0)
    { /* Insert CODE IS value */
...
do_page_heading(cob_report *r)
{
    ...
    rec = (char *)f->record->data;
    memset(rec, ' ', f->record_max);
    if (!r->in_page_heading && !r->first_generate && r->def_lines > 0 && r->def_heading > 0 && r->curr_line <= r->def_lines && r->curr_line > r->def_heading)
    { /* Skip to end of page */
    ...
    { /* Skip to Heading position on page */
        write_rec(r, opt);
        r->curr_line++;
        saveLineCounter(r);
    }
...
print_field(cob_report_field *rf, char *rec)
{
    ...
    if (cobsetptr && !cobsetptr->cob_col_just_lrc)
    {
        /* Data justify is turned off, no adjustment */
    }
    else if ((rf->flags & COB_REPORT_COLUMN_RIGHT) && ln < rf->f->size)
    {
        dest_pos += rf->f->size - ln;
    }
    else if ((rf->flags & COB_REPORT_COLUMN_CENTER))
    {
        for (k = 0; k < rf->f->size && wrk[0] == ' ' && ln > 0; k++)
    ...
    else if ((rf->flags & COB_REPORT_COLUMN_LEFT))
    {
        for (k = 0; k < rf->f->size && wrk[0] == ' ' && ln > 0; k++)
        { /* remove leading spaces */
            memmove(wrk, &wrk[1], ln);
...
report_line(cob_report *r, cob_report_line *l)
{
    ...
    if (rec)
    {
...
#endif
        if (r->curr_line > r->def_last_detail && !r->in_report_footing && !r->in_page_footing)
        { /* Page overflow */
            do_page_footing(r);
            do_page_heading(r);
        }
        ...
        else if (!r->next_just_set && r->next_line)
        {
            DEBUG_LOG("rw", (" Line# %d of Page# %d; ", r->curr_line, r->curr_page));
        ...
        else if (!r->next_just_set && r->next_page)
        {
            DEBUG_LOG("rw", (" Line# %d of Page# %d; ", r->curr_line, r->curr_page));
        ...
        else if (!(l->flags & COB_REPORT_LINE_PLUS) && (l->flags & COB_REPORT_LINE))
        {
        ...
        if (bChkLinePlus && (l->flags & COB_REPORT_LINE_PLUS) && l->line > 1)
        {
            if (r->curr_line != r->def_first_detail || r->def_first_detail == 0)
            {
        ...
        bChkLinePlus = FALSE;
        if (r->curr_line > r->def_last_detail && !r->in_report_footing && !r->in_page_heading && !r->in_page_footing)
        { /* Page overflow */
            do_page_footing(r);
            do_page_heading(r);
        }
        ...
        for (rf = l->fields; rf; rf = rf->next)
        {
            ...
            if ((rf->flags & COB_REPORT_PRESENT) && !rf->present_now)
            {
                continue;
            }
            ...
            if (rf->source)
            { /* Copy source field in */
                cob_move(rf->source, rf->f);
                print_field(rf, rec);
            }
            else if (rf->litval)
            { /* Refresh literal value */
                if (rf->f)
                {
            ...
            if ((rf->flags & COB_REPORT_GROUP_INDICATE))
            { /* Suppress subsequent printings */
...
report_line_type(cob_report *r, cob_report_line *l, int type)
{
    ...
    if (l->flags & type)
    {
        ...
        if (l->sister)
        {
            if ((type == COB_REPORT_CONTROL_FOOTING) && (l->sister->flags & COB_REPORT_CONTROL_FOOTING))
            {
                curseq = get_control_sequence(r, l);
                sisseq = get_control_sequence(r, l->sister);
                if (curseq > 0 && sisseq > 0 && sisseq > curseq)
                {
#ifdef COB_DEBUG_LOG
    ...
    for (sc = r->sum_counters; sc; sc = sc->next)
    {
        for (rs = sc->sum; rs; rs = rs->next)
        {
            ...
            else if (sc->control)
            {
                ...
                for (rr = rc->control_ref; rr; rr = rr->next)
                {
                    if (rr->ref_line && (rr->ref_line->flags & COB_REPORT_CONTROL_HEADING))
                        continue;
                    if (rr->ref_line && (rr->ref_line->flags & COB_REPORT_CONTROL_HEADING_FINAL))
                        continue;
                    if (l != NULL && l != get_print_line(rr->ref_line))
                        continue;
                    if (rr->ref_line && (rr->ref_line->flags & flag))
                    {
 ...
 * Runtime starting up
 */
void cob_init_reportio(cob_global *gptr, cob_settings *sptr)
{
    int k;
    cobglobptr = gptr;
    cobsetptr = sptr;
 ...
 * INITIATE report
 */
void cob_report_initiate(cob_report *r)
{
    cob_report_control *rc;
    cob_report_control_ref *rr;
    cob_report_sum_ctr *sc;
    ...
    if (r->def_cols > REPORT_MAX_COLS || r->def_cols < 1)
        r->def_cols = REPORT_MAX_COLS;
    if ((r->def_first_detail > 0 && !(r->def_first_detail >= r->def_heading)) || (r->def_last_detail > 0 && !(r->def_last_detail >= r->def_first_detail)) || (r->def_footing > 0 && !(r->def_footing >= r->def_heading)) || (r->def_footing > 0 && !(r->def_footing >= r->def_last_detail)) || (r->def_lines > 0 && !(r->def_lines >= r->def_heading)) || (r->def_lines > 0 && !(r->def_lines >= r->def_footing)))
    {
 ...
 * TERMINATE report
 */
int cob_report_terminate(cob_report *r, int ctl)
{
    cob_report_control *rc;
    cob_report_control_ref *rr;
    cob_report_line *pl;
    ...
    if (ctl > 0)
    { /* Continue Processing Footings from last point */
        for (rc = r->controls; rc; rc = rc->next)
        {
            for (rr = rc->control_ref; rr; rr = rr->next)
            {
                if (rr->ref_line->flags & COB_REPORT_CONTROL_FOOTING)
                {
                    ...
                    if (pl != rr->ref_line && pl->use_decl == ctl)
                    {
                        DEBUG_LOG("rw", ("  Continue after Declaratives %d\n", ctl));
                        goto PrintFooting; /* Continue Footings */
                    }
 ...
 */
int cob_report_generate(cob_report *r, cob_report_line *l, int ctl)
{
    cob_report_control *rc;
    cob_report_control_ref *rr;
    cob_report_line *pl;
    ...
    if (r->first_generate)
    {
        ...
        if (r->first_line->use_decl && r->first_line->use_decl != last_use)
        {
    ...
    else
    {
        ...
        for (rc = r->controls; rc; rc = rc->next)
        {
            ...
            if (rc->data_change)
            { /* Data change, implies control break at lower levels */
#if defined(COB_DEBUG_LOG)
        ...
        if (maxctl > 0)
        {
            ...
            {
                if (rp->sequence < maxctl && !rp->data_change)
                {
                    rp->data_change = TRUE;
        ...
        {
            if (rc->data_change)
            { /* Data change, Check for PRESENT WHEN control-id */
        ...
        for (rc = r->controls; rc; rc = rc->next)
        {
            if (rc->data_change)
            {
                for (rr = rc->control_ref; rr; rr = rr->next)
                {
                    ...
                    PrintFooting:
                        if (!rc->suppress && !rr->ref_line->suppress)
                            report_line_and(r, rr->ref_line, COB_REPORT_CONTROL_FOOTING);
                        rc->suppress = FALSE;
    ...
    else
    {
        ...
        if (l->fields == NULL && l->child != NULL && l->child->sister != NULL)
        {
            if (l->use_decl && l->use_decl != last_use)
            {
        ...
        for (pl = l; pl; pl = pl->sister)
        {
            ...
            if ((pl->flags & COB_REPORT_LINE_PLUS) && pl->line > 1)
            {
                ln += pl->line;
            }
            num++;
        ...
        if (num > 1 && (r->curr_line + ln) > r->def_last_detail)
        { /* Page overflow */
            do_page_footing(r);
            r->curr_line = 1;
            do_page_heading(r);
 ...
 */
void cob_report_suppress(cob_report *r, cob_report_line *l)
{
    cob_report_control *rc;
    cob_report_control_ref *rr;
    cob_report_line *pl;
