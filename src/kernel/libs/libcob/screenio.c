...
static void init_cob_screen_if_needed(void)
{
    if (!cobglobptr)
    {
...
handle_status(const int fret, const enum screen_statement stmt)
{
    if (fret)
    {
        cob_set_exception(stmt == ACCEPT_STATEMENT ? COB_EC_IMP_ACCEPT : COB_EC_IMP_DISPLAY);
    }
...
void cob_set_cursor_pos(int line, int column)
{
                               ...
                               short *fg_color, short *bg_color)
{
    ...
    for (i = 0; i < max_tokens; i++)
    {
        ...
        if (len == 5 && memcmp(keyword, "ERASE", 5) == 0)
        {
            ...
            else if (len == 3 && memcmp(keyword, "EOS", 3) == 0)
            {
                keyword = "ERASE EOS";
            }
            else
        ...
        if (len == 5 && memcmp(keyword, "BLANK", 5) == 0)
        {
            ...
            else if (len == 6 && memcmp(keyword, "SCREEN", 6) == 0)
            {
        ...
        /* find token and get its attribute */
        {
            ...
            if (control_attr == NULL || control_attr->cobflag == -1)
            {
                continue;
            }
        ...
        if ((len == 2 && memcmp(keyword, "IS", 2) == 0) || (len == 1 && *keyword == '='))
        {
            if (++i == max_tokens)
            {
                break;
...
get_cursor_from_program(int *line, int *column)
{
    if (COB_MODULE_PTR && COB_MODULE_PTR->cursor_pos)
    {
        int ret = get_line_and_col_from_field(COB_MODULE_PTR->cursor_pos, line, column);
        /* LCOV_EXCL_START */
        if (unlikely(ret == 1))
        {
            cob_fatal_error(COB_FERROR_CODEGEN);
    ...
    */
    for (; s; s = get_prev_screen_item(s))
    {
        ...
        if (!found_col)
        {
            ...
            if (!found_col && !s->column && is_elementary && !is_first_screen_item(s))
            {
                /*
...
{
    return (COB_FIELD_IS_NUMERIC(field) || COB_FIELD_TYPE(field) == COB_TYPE_NUMERIC_EDITED);
}
...
field_is_zero(cob_screen *s)
{
    ...
    for (i = 0; i < size; ++i)
    {
        if (!(isspace(data[i]) || data[i] == '0' || data[i] == decimal_point))
        {
            return 0;
        }
    }
...
get_num_int_digits_for_no_zero_sup(const cob_pic_symbol *pic)
{
    ...
    for (i = 0; pic[i].symbol != '\0'; ++i)
    {
        if (pic[i].symbol == '9' || pic[i].symbol == 'Z' || pic[i].symbol == '*')
        {
            num_digits += pic[i].times_repeated;
        }
        else if (!(pic[i].symbol == numeric_separator || pic[i].symbol == 'B' || pic[i].symbol == '0' || pic[i].symbol == '/') && num_digits != 0)
        {
            break;
        }
    }
...
mouse_to_exception_code(mmask_t mask)
{
    ...
    if (mask & BUTTON1_PRESSED)
        fret = 2041;
    else if (mask & BUTTON1_CLICKED)
        fret = 2041;
    else if (mask & BUTTON1_RELEASED)
        fret = 2042;
    else if (mask & BUTTON1_DOUBLE_CLICKED)
        fret = 2043;
    else if (mask & BUTTON1_TRIPLE_CLICKED)
        fret = 2043;
    else if (mask & BUTTON2_PRESSED)
        fret = 2044;
    else if (mask & BUTTON2_CLICKED)
        fret = 2044;
    else if (mask & BUTTON2_RELEASED)
        fret = 2045;
    else if (mask & BUTTON2_DOUBLE_CLICKED)
        fret = 2046;
    else if (mask & BUTTON2_TRIPLE_CLICKED)
        fret = 2046;
    else if (mask & BUTTON3_PRESSED)
        fret = 2047;
    else if (mask & BUTTON3_CLICKED)
        fret = 2047;
    else if (mask & BUTTON3_RELEASED)
        fret = 2048;
    else if (mask & BUTTON3_DOUBLE_CLICKED)
        fret = 2049;
    else if (mask & BUTTON3_TRIPLE_CLICKED)
        fret = 2049;
...
{
    return has_decimal_point(f) && ccolumn == offset + scolumn + get_decimal_point_offset(f);
}
                ...
                const int ccolumn)
{
    if (cob_field_is_numeric_or_numeric_edited(f))
    {
        ...
        {
            return leftmost_char == '0' || leftmost_char == ' ';
        }
    }
    else
    {
...
cob_screen_get_all(const int initial_curs, const int accept_timeout)
{
    ...
    for (;;)
    {
        ...
        if (keyp == KEY_MOUSE)
        {
            ...
            if (mevent.bstate & BUTTON1_DOUBLE_CLICKED && !(cob_mask_accept & BUTTON1_DOUBLE_CLICKED))
            {
                keyp = KEY_ENTER;
            }
        }
        ...
        case KEY_BACKSPACE:
            ...
            if (ccolumn > scolumn && !at_offset_from_decimal_point(s->field, scolumn, ccolumn, 1))
            {
                at_eof = 0;
        ...
        case '.':
        case ',':
            if (keyp != COB_MODULE_PTR->decimal_point || !has_decimal_point(s->field))
            {
                break;
            }
        ...
        case KEY_MOUSE:
        {
            ...
            if (mevent.bstate & BUTTON1_PRESSED && COB_MOUSE_FLAGS & 1)
            {
                int fld_index = -1;
                /* if in current field, just move */
                if (mline == cline)
                {
                    if (mcolumn >= scolumn && mcolumn <= right_pos)
                    {
                        ccolumn = mcolumn;
                        cob_move_cursor(cline, ccolumn);
                        p = s->field->data + ccolumn - scolumn;
        ...
        if (keyp > 037 && keyp <= UCHAR_MAX)
        {
#endif
            /* Numeric field check. */
            if (cob_field_is_numeric_or_numeric_edited(s->field) && ((keyp != '+' && keyp != '-' && (keyp < '0' || keyp > '9')) || at_offset_from_decimal_point(s->field, scolumn, ccolumn, 0)))
            {
                cob_beep();
                continue;
            }
            ...
            if (COB_INSERT_MODE)
            {
                ...
                if (cob_field_is_numeric_or_numeric_edited(s->field))
                {
                    ...
                    if (!has_dp || ccolumn < integer_part_end)
                    {
                        /*
            ...
            *p = (unsigned char)keyp;
            ...
            if (ccolumn == right_pos)
            {
                ...
                {
                    at_eof = !COB_INSERT_MODE || !can_insert_left(s->field, *s->field->data, ccolumn);
                }
            }
    ...
    if (column == NULL)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                int ret = get_line_and_col_from_field(line, &cobol_line, &cobol_col);
                /* LCOV_EXCL_START */
                if (unlikely(ret == 1))
                {
              ...
              cob_field *size_is, cob_field *control, cob_field *color)
{
    ...
    if (!(fattr & COB_SCREEN_NO_DISP))
    {
        ...
        if (size_is && f->attr->type == COB_TYPE_ALPHANUMERIC_ALL)
        {
            if ((int)f->size == 1)
            {
                fig_const = f->data[0];
        ...
        raise_ec_on_truncation(size_accept);
        {
            ...
            /* start position within the field, if specified (all 1-based) */
            {
                ...
                else
                {
                    ...
                    if (cursor_clause_line == sline && cursor_clause_col > scolumn && cursor_clause_col < scolumn + disp_size)
                    {
    ...
    for (;;)
    {
        ...
        case KEY_MOUSE:
        {
            ...
            if (mevent.bstate & BUTTON1_PRESSED && COB_MOUSE_FLAGS & 1)
            {
                ...
                if (mline == cline)
                {
                    if (mcolumn >= scolumn && mcolumn <= (int)right_pos)
                    {
                        ccolumn = mcolumn;
                        cob_move_cursor(cline, ccolumn);
                        p = COB_TERM_BUFF + ccolumn - scolumn;
...
void cob_display_field(cob_field *f, const cob_flags_t fattr, const char *parms, ...)
{
    cob_field *line = NULL;
...
void cob_accept_field(cob_field *f, const cob_flags_t fattr, const char *parms, ...)
{
    cob_field *line = NULL;
    cob_field *column = NULL;
    cob_field *fgc = NULL;
...
void cob_field_display(cob_field *f, cob_field *line, cob_field *column,
                       cob_field *fgc, cob_field *bgc, cob_field *fscroll,
                       cob_field *size_is, const cob_flags_t fattr)
{
    const int zero_line_col_allowed = 1; /* see comment in cob_display_field */
...
void cob_field_accept(cob_field *f, cob_field *line, cob_field *column,
                      cob_field *fgc, cob_field *bgc, cob_field *fscroll,
...
int cob_display_text(const char *text)
{
    cob_field field;
    cob_field_attr attr;
...
int cob_get_text(char *text, int size)
{
    cob_field field;
    cob_field_attr attr;
...
int cob_display_formatted_text(const char *fmt, ...)
{
    int size;
    cob_field field;
    cob_field_attr attr;
...
void cob_exit_screen_from_signal(int ss_only)
{
...
#if (!defined(NCURSES_VERSION_MAJOR) || NCURSES_VERSION_MAJOR < 6) && (!defined(PDC_BUILD) || PDC_BUILD < 4305)
    if (ss_only)
        return;
#endif
...
void cob_exit_screen_from_signal(int signal_safe_only)
{
    /* nothing possible to do here */
...
/* ACCEPT scr-name */
void cob_screen_accept(cob_screen *s, cob_field *line,
...
void cob_screen_line_col(cob_field *f, const int get_columns)
{
    init_cob_screen_if_needed();
#ifdef WITH_EXTENDED_SCREENIO
    if (get_columns)
...
#ifdef WITH_EXTENDED_SCREENIO
    if (!cobglobptr->cob_screen_initialized && COB_BEEP_VALUE != 2)
    {
        int ret = cob_screen_init();
        if (ret)
        {
...
*/
int cob_sys_get_char(unsigned char *fld)
{
#ifdef WITH_EXTENDED_SCREENIO
    int ret;
#endif
...
int cob_sys_set_csr_pos(unsigned char *fld)
{
#ifdef WITH_EXTENDED_SCREENIO
    const cob_field *f = COB_MODULE_PTR->cob_procedure_params[0];
    int cline;
...
int cob_sys_set_scr_size(unsigned char *line, unsigned char *col)
{
