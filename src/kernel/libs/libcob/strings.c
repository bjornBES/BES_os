...
unsigned char * inspect_find_data(const cob_field *str)
{
...
set_inspect_mark(const size_t pos, const size_t length)
{
    ...
    if ((inspect_mark_min == 0 && inspect_mark[inspect_mark_min] == 0) || pos < inspect_mark_min)
    {
...
is_marked(size_t pos, size_t length)
{
    ...
    if (inspect_mark[inspect_mark_min] == 0 || inspect_mark_max < pos || inspect_mark_min >= pos + length)
    {
        return 0;
    }
    /* ... or if the minimal/max mark are within the range to check */
    if (inspect_mark_min >= pos || inspect_mark_max < pos + length)
    {
        return 1;
    }
...
void cob_inspect_init_converting(cob_field *var)
{
    cob_inspect_init_common(var);
...
void cob_inspect_finish(void)
{
    ...
    if (inspect_replacing && inspect_repdata_size != 0 /* check for first INSPECT REPLACING having zero length */
...
void cob_init_strings(cob_global *lptr)
{
    cobglobptr = lptr;
