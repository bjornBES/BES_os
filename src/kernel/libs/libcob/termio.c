...
display_numeric(cob_field *f, fd_t fp)
{
    ...
    const int size = digits + ((scale < 0) ? scale : 0) /* subtract scale when negative */
                     + has_sign;
...
clean_double(char *wrk)
{
    ...
    if (strcmp(wrk, "-NAN") == 0 || strcmp(wrk, "-NaNQ") == 0 || strcmp(wrk, "-NaN") == 0 || strcmp(wrk, "NAN") == 0 || strcmp(wrk, "NaNQ") == 0)
    {
        strcpy(wrk, "NaN");
    }
}
...
void cob_display_common(const cob_field *f, fd_t fp)
{
    ...
    if (COB_FIELD_IS_NUMERIC(f))
    {
        ...
        if (COB_FIELD_REAL_BINARY(f) || (COB_FIELD_TYPE(f) == COB_TYPE_NUMERIC_BINARY && !COB_MODULE_PTR->flag_pretty_display))
        {
...
void cob_display(const int to_device, const int newline, const int varcnt, ...)
{
    ...
    case 3: /* SYSPCH */
        ...
        if (!cobsetptr->cob_display_punch_file && cobsetptr->cob_display_punch_filename != NULL)
        {
            const char *mode;
            if (!cobsetptr->cob_unix_lf)
            {
...
display_alnum_dump(cob_field *f, fd_t fp, unsigned int indent, unsigned int max_width)
{
    ...
    for (i = 0; i < fsize; i++)
    {
        ...
        else if (f->data[i] == '\b' || f->data[i] == '\f' || f->data[i] == '\n' || f->data[i] == '\r' || f->data[i] == '\t')
        {
            delv++;
        }
        else if (f->data[i] >= ' ' && isprint(f->data[i]))
        {
            printv++;
        }
    }
        ...
        for (i = 0; len > colsize; i += colsize, len -= colsize)
        {
            if (colsize < MAX_PREV)
            {
                ...
                else if (memcmp(prev, &f->data[i], colsize) == 0)
                {
                    duplen = 0;
                    bgn = i + 1;
                    while (memcmp(prev, &f->data[i], colsize) == 0 && len > colsize)
                    {
                        duplen += colsize;
                        i += colsize;
                        len -= colsize;
    ...
    for (i = 0; i < fsize;)
    {
        if (colsize < MAX_PREV && i < (fsize - colsize))
        {
            if (i != 0 && memcmp(prev, &f->data[i], colsize / 2) == 0)
            {
                duplen = 0;
                bgn = i;
                while (memcmp(prev, &f->data[i], colsize / 2) == 0 && i < (fsize - colsize / 2))
                {
                    duplen += colsize / 2;
                    i += colsize / 2;
                }
                    ...
                    const unsigned int indexes, va_list ap)
{
    ...
    if (indexes != 0)
    {
        ...
        for (cob_idx = 1; cob_idx <= indexes; cob_idx++)
        {
            ...
            if (!dump_compat && dump_skip[cob_idx] == 0 && dump_skip[cob_idx - 1] == cob_subscript)
            {
                return;
            }
    ...
    if (!dump_compat)
    {
        ...
        if (calc_dump_index < dump_index || dump_idx[calc_dump_index] > subscript[calc_dump_index])
        {
            dump_pending_output(fp);
            for (;;)
            {
        ...
        if (indexes != 0)
        {
            ...
            if (dump_idx[dump_index] != subscript[dump_index])
            {
                ...
                if (dump_prev_data[dump_index] != NULL && memcmp(dump_prev_data[dump_index], data, f->size) == 0)
                {
    ...
    else
    {
        ...
        if ((COB_FIELD_TYPE(f) == COB_TYPE_NUMERIC_EDITED || COB_FIELD_TYPE(f) == COB_TYPE_NUMERIC_DISPLAY) && !is_field_display(f))
        {
            display_alnum_dump(f, fp, 41, cobsetptr->cob_dump_width);
        }
        else if (COB_FIELD_TYPE(f) == COB_TYPE_ALPHANUMERIC || COB_FIELD_TYPE(f) == COB_TYPE_ALPHANUMERIC_EDITED || f->size > 39)
        {
            display_alnum_dump(f, fp, 41, cobsetptr->cob_dump_width);
        }
        else
...
/* 3.1 - compat function */
void cob_dump_field(const int level, const char *name, cob_field *f_addr,
                    const int offset_ext, const int indexes_ext, ...)
{
    ...
    if (level < 0)
    { /* Special directive, only included for 3.1rc1 - compat */
        if (level == -1)
        {
...
void cob_dump_field_ext(const int level, const char *name, cob_field *f_addr,
